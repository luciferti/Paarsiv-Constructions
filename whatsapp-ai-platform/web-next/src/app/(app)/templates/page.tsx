...
import { GalleryHorizontalEnd, Heading, Image as ImageIcon, Pencil, Trash2, X } from "lucide-react";
      ...
      <section className="flex-1 min-w-0 flex flex-col bg-background">
        <div className="px-6 py-4 border-b bg-card/50 flex items-center gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Reusable messages. Use {"{{name}}"} and {"{{city}}"} to personalize.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4 content-start">
          {shown.map((t) => (
            <div key={t.id} className="rounded-xl border bg-card shadow-card p-4 flex flex-col gap-2">
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold text-sm truncate">{t.name}</span>
                <span className="text-[10px] px-1.5 py-px rounded bg-accent text-accent-foreground font-medium shrink-0">{t.category.toLowerCase()}</span>
              </div>
              {t.type === "carousel" ? (
                <span className="inline-flex items-center gap-1 text-[11px] text-warning">
                  <GalleryHorizontalEnd className="w-3 h-3" /> carousel · {t.cards.length} cards
                </span>
              ) : t.headerType !== "none" && (
                <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                  {t.headerType === "text" ? <Heading className="w-3 h-3" /> : <ImageIcon className="w-3 h-3" />} {t.headerType} header
                </span>
              )}
